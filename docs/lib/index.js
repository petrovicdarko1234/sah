...
function canMoveFunc(_selected, target, _pieceMatrix) {
    let canMove = false;
    if (target.rank != "" && _selected.white == target.white) {
        return false;
    }
...
function makeMove(select, target) {
    if (target == select) {
        return;
    }
    resetAudio();
...
function handlePawn(select, target, matrix) {
    let startI = 6;
    let offsetI = -1;
    if (!select.white) {
        offsetI = 1;
...
function handleRook(selected, target, matrix) {
...
function handleBishop(selected, target, matrix) {
    let difI = selected.i - target.i;
    let difJ = selected.j - target.j;
    if (selected.rank == "king") {
        if (Math.abs(difI) != 1 || Math.abs(difJ) != 1) {
...
function handleKnight(selected, target) {
